...
void handle_client_send(int client_fd, const char *target_path, const char *base_cache) {
    ...
    // 2. Create the final destination folder using the passed base_cache
    char dest_dir[8192];
...
void handle_client_exec(int client_fd, const char *target_path, int save_to_server, const char *base_cache) {
    ...
    // 1. IMPORTANT: Inject the cache path as an Environment Variable!
    ...
    // 2. Create a highly secure, randomized temporary file for the script
